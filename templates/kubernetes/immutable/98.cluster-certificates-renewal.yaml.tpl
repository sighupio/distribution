...
#
# Check for expiration first e.g. via executing the following on a control_plane node via:
#
...
- name: Renew Kubernetes PKI certificates
  ...
  tasks:
    ...
    - name: Ensure Kubernetes backup directory exists
      file:
        path: "{{ "{{ ansible_facts['env']['HOME'] }}" }}/certs-backup/{{ print "{{ backup_timestamp }}" }}/etc-kubernetes-pki"
        state: directory
        mode: '0750'
      when: inventory_hostname in groups['control_plane']
    ...
    - name: Backup Kubernetes certificates
      copy:
        ...
        dest: "{{ "{{ ansible_facts['env']['HOME'] }}" }}/certs-backup/{{ print "{{ backup_timestamp }}" }}/etc-kubernetes-pki"
        remote_src: yes
        mode: preserve
        directory_mode: '0750'
      when: inventory_hostname in groups['control_plane']
    ...
    - name: Backup etcd certificates
      copy:
        src: /etc/etcd/pki/
        dest: "{{ "{{ ansible_facts['env']['HOME'] }}" }}/certs-backup/{{ print "{{ backup_timestamp }}" }}/etc-etcd-pki"
        remote_src: yes
        mode: preserve
        directory_mode: '0750'
      when: inventory_hostname in groups['etcd']
...
- name: Distribute etcd certificates from etcd to control plane nodes
  ...
  tasks:
    - name: Retrieving certificates from etcd nodes
      ...
      delegate_to: "{{ "{{ groups.etcd[0] | default(omit) }}" }}"
      fetch:
