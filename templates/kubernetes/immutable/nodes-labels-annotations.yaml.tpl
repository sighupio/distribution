  ...
  tasks:
    - name: Format control-plane labels and annotations for usage with kubectl
      vars:
        ...
        removed_cp_labels: "{{ "{{ furyctl_yaml.spec.kubernetes.controlPlane.labels | default([], true) | difference(kubernetes_node_labels|default([], true)) | reject('match', 'node-role.kubernetes.io/control-plane') }}" }}"
        removed_cp_annotations: "{{ "{{ furyctl_yaml.spec.kubernetes.controlPlane.annotations | default([], true) | difference(kubernetes_node_annotations|default([], true)) }}" }}"
      ansible.builtin.set_fact:
...
- name: Preparing nodes labels and annotations
  ...
  tasks:
    - name: Format nodes labels and annotations for usage with kubectl
      vars:
        # For the nodes we can't access directly the labels and annotations properties like we do for the control plane members
        # because nodes is a list of node groups.
        # We need to identify which element of the `nodes` list is the right one for this node.
        node_group_details: "{{ "{{ furyctl_yaml.spec.kubernetes.nodeGroups | selectattr('name', '==', kubernetes_role) | first }}" }}"
...
- name: Update control-plane and nodes labels and annotations
  ...
  tasks:
    ...
    - name: Set nodes role based on the node group's name
      delegate_to: localhost
      ansible.builtin.command: "{{ .paths.kubectl }} {{ "label node {{ inventory_hostname }} node-role.kubernetes.io/{{ kubernetes_role }}= --kubeconfig={{ kubernetes_kubeconfig_path }}admin.conf" }}"
    ...
    - name: Update node labels
      delegate_to: localhost
      ansible.builtin.command: "{{ .paths.kubectl }} {{ "label node {{ inventory_hostname }} {{ node_labels }} --overwrite --kubeconfig={{ kubernetes_kubeconfig_path }}admin.conf" }}"
    ...
    - name: Update node annotations
      delegate_to: localhost
      ansible.builtin.command: "{{ .paths.kubectl }} {{ "annotate node {{ inventory_hostname }} {{ node_annotations }} --overwrite --kubeconfig={{ kubernetes_kubeconfig_path }}admin.conf" }}"
