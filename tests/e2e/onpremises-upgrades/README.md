...
upgrades it to **v1.35.1** with `furyctl`, running the distribution checks after
each step. Same local libvirt infra as the [install e2e](../onpremises/README.md);
this one exercises the real upgrade path.

Runs in CI via the `e2e-onpremises-upgrades-1.34.1-1.35.1` pipeline in `.drone.yml`.
...
1. **install-tools**, **provision-vms**, **prepare-nodes** — as the install e2e, but
   `provision-vms` also renders `furyctl_upgrade.yaml` (the v1.35.1 target config).
2. **install-1.34.1** — `furyctl apply` of the v1.34.1 base config.
3. **bats-test-1.34.1** — distribution checks on the base cluster.
4. **upgrade-1.35.1** — `furyctl apply --upgrade --force upgrades --config furyctl_upgrade.yaml`.
5. **bats-test-1.35.1** — distribution checks after the upgrade (incl. haproxy ingress).
...
onpremises-upgrades/
  tofu/     calls the shared vms module; output.tf renders furyctl.yaml (v1.34.1)
            and furyctl_upgrade.yaml (v1.35.1)
