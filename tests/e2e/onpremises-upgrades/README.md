...
Installs the distribution at **v1.33.2** on a throwaway HA cluster of KVM VMs, then
upgrades it to **v1.33.3** with `furyctl`, running the distribution checks after
each step. Same local libvirt infra as the [install e2e](../onpremises/README.md);
this one exercises the real upgrade path.

Runs in CI via the `e2e-onpremises-upgrades-1.33.2-1.33.3` pipeline in `.drone.yml`.
...
1. **install-tools**, **provision-vms**, **prepare-nodes** — as the install e2e, but
   `provision-vms` also renders `furyctl_upgrade.yaml` (the v1.33.3 target config).
2. **install-1.33.2** — `furyctl apply` of the v1.33.2 base config.
3. **bats-test-1.33.2** — distribution checks on the base cluster.
4. **upgrade-1.33.3** — `furyctl apply --upgrade --force upgrades --config furyctl_upgrade.yaml`.
5. **bats-test-1.33.3** — distribution checks after the upgrade (incl. haproxy ingress).
6. **delete** — `tofu destroy` (always, on success or failure).

## Layout
```
onpremises-upgrades/
  tofu/     calls the shared vms module; output.tf renders furyctl.yaml (v1.33.2)
            and furyctl_upgrade.yaml (v1.33.3)
