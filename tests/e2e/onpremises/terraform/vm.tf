...
variable "server_type" {
  ...
  # ccx types are quota-limited). Avoid the EOL AMD line (cpx*, no longer
  # orderable) and the capacity-constrained ARM line (cax*, frequent
  # resource_unavailable). cx52 = current-gen Intel shared, 16 vCPU / 32 GB.
  # arm64 alt (if you hit it): cax41 — the vabbe/KFD images are multi-arch.
  default = "cx52"
}
