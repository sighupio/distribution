...
REPO_ROOT="$(cd "$ONPREM/../../.." && pwd)"
cd "$ONPREM/config"

: "${GITHUB_TOKEN:?export GITHUB_TOKEN (e.g. GITHUB_TOKEN=\$(gh auth token)) — furyctl/mise need it}"

# config/ lives inside the distribution repo, so mise also loads the repo-root
# mise.toml hierarchically. Trust the whole repo so parsing doesn't fail, and
# disable auto-install so `mise exec` doesn't pull the full toolchain on this
# host — we only need vabbe here (furyctl/kubectl/ansible run in the runner).
export MISE_TRUSTED_CONFIG_PATHS="$REPO_ROOT"
export MISE_NOT_FOUND_AUTO_INSTALL=false

echo "### 0. tools (vabbe on this host; runner toolchain installs in the runner)"
mise install github:nutellinoit/vabbe
