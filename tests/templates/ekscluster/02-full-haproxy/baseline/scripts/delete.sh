...
echo "Ingresses deleted"publicHostedZones=""
...
aws resourcegroupstaggingapi get-resources --region eu-west-1 --resource-type-filters elasticloadbalancing:loadbalancer --tag-filters "Key=elbv2.k8s.aws/cluster,Values=full-cilium-haproxy" --query 'ResourceTagMappingList[].ResourceARN' --output text 2>/dev/null > ./lb_arns.txt || true
