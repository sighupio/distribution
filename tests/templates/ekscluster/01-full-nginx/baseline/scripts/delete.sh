...
echo "Ingresses deleted"publicHostedZones=""privateHostedZones=""
...
aws resourcegroupstaggingapi get-resources --region eu-west-1 --resource-type-filters elasticloadbalancing:loadbalancer --tag-filters "Key=elbv2.k8s.aws/cluster,Values=full-calico-nginx" --query 'ResourceTagMappingList[].ResourceARN' --output text 2>/dev/null > ./lb_arns.txt || true
...
if [ -f ./lb_arns.txt ]; then
  for arn in $(cat ./lb_arns.txt); do
    aws elbv2 describe-load-balancers --region eu-west-1 --load-balancer-arns "$arn" >/dev/null 2>&1 && \
      aws elbv2 delete-load-balancer --region eu-west-1 --load-balancer-arn "$arn" || true
  done
  rm -f ./lb_arns.txt
fi
