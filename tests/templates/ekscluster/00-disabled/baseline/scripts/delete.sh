...
aws resourcegroupstaggingapi get-resources --region eu-west-1 --resource-type-filters elasticloadbalancing:loadbalancer --tag-filters "Key=elbv2.k8s.aws/cluster,Values=no-modules-enabled" --query 'ResourceTagMappingList[].ResourceARN' --output text 2>/dev/null > ./lb_arns.txt || true
