...
This patch release backports the `v1.35.0` and `v1.35.1` module updates and features to the `v1.34` line, while keeping **Kubernetes 1.34** as the supported minor version. It targets Kubernetes `v1.34.8` on-premises (bumped from `1.34.4` in `v1.34.1`) and continues to support EKS `1.34`.
...
- [on-premises](https://github.com/sighupio/installer-on-premises) 📦 installer: [**v1.35.5**](https://github.com/sighupio/installer-on-premises/releases/tag/v1.35.5)
  - Installs Kubernetes version 1.34.8 and adds support for Kubernetes v1.33.12.
...
- [eks](https://github.com/sighupio/installer-eks) 📦 installer: [**v3.5.0**](https://github.com/sighupio/installer-eks/releases/tag/v3.5.0)
  - Verified compatibility with Kubernetes 1.34.
...
- [[#539](https://github.com/sighupio/distribution/pull/539)] Pin `ansible` (with its `python` + `uv` build toolchain and the galaxy collections) under `tools.common` in `kfd.yaml`, so `furyctl` installs a self-contained, versioned Ansible via the bundled mise for the `OnPremises` kind instead of requiring it on the host. Optional and backward compatible: distributions without the block keep using the host Ansible.
