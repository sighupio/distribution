...
- [[#505](https://github.com/sighupio/distribution/pull/505)] On-premises: the `spec.kubernetes.advanced.kubeProxy.enabled` boolean has been removed in favor of `spec.kubernetes.advanced.kubeProxy.type` (`none` | `ipvs` | `nftables`, default `ipvs`). Replace `enabled: false` with `type: none`; `enabled: true` (or unset) maps to `type: ipvs`.
