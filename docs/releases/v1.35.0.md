...
- [[#505](https://github.com/sighupio/distribution/pull/505)] To support NFTables mode for kube-proxy, the boolean `kubeProxy.enabled` field is removed in favor of the `kubeProxy.type` enum (`ipvs`, `nftables` or `none`). Migrate existing configurations as follows: `enabled: false` becomes `type: none`; `enabled: true` (or the field omitted) becomes `type: ipvs`, which is also the default when `type` is omitted.
